...
class PersonaGUI:
    ...
    def show_sidebar(self):
        ...
        with st.sidebar:
            ...
            for cluster_id, persona in self.personas.items():
                ...
                with col2:
                    if st.button(f"**{persona['name']}**", key=f"persona_{cluster_id}", use_container_width=True):
                        selected_persona = cluster_id
            ...
            if 'selected_persona' in st.session_state:
                ...
                st.markdown("### 📊 지식베이스")
                st.metric("전사본 수", f"{stats['total_transcripts']}개")
                st.metric("상위 키워드", f"{len(stats['top_keywords'])}개")
        ...
        if st.button("💡 라이프스타일 팁 생성", use_container_width=True):
            with st.spinner(f"{persona['name']}의 라이프스타일 팁을 생성 중..."):
                ...
                <div class='lifestyle-tip'>
                    ...
                    <div style='white-space: pre-wrap;'>{tips}</div>
                </div>
                """, unsafe_allow_html=True)
        ...
        for category in categories:
            if st.button(f"📋 {category}", use_container_width=True):
                with st.spinner(f"{category} 가이드 생성 중..."):
                    ...
                    <div class='lifestyle-tip'>
                        <h4>📋 {category}</h4>
                        <div style='white-space: pre-wrap;'>{guide}</div>
                    </div>
                    """, unsafe_allow_html=True)
    ...
    def show_content_creation(self):
                ...
                if content_topic:
                    with st.spinner("콘텐츠 아이디어 생성 중..."):
                        ...
                        <div class='content-idea'>
                            ...
                            <hr>
                            <div style='white-space: pre-wrap;'>{idea}</div>
                        </div>
                        """, unsafe_allow_html=True)
        ...
        for content_type in types:
            if st.button(f"🎬 {content_type} 아이디어", use_container_width=True):
